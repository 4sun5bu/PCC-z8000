...
short cinfo[128] = {
	...
	ERR,	ERR,	ERR,	ERR,	ERR,	ERR,	ERR,	ERR,
	ERR,	ERR,	ERR,	ERR,	ERR,	ERR,	ERR,	ERR,
	SPC,	EOL,	QUO,	IMM,	S+T,	ERR,	ERR,	CHR,
 ...
 * For Z8000, we need to handle two-word opcodes like "jr eq"
 * by looking up the full mnemonic including condition.
 */
int sopcode(token)
  register char *token;
  {	register char *p;
	...
	if (p - mnem == 2 && mnem[0] == 'j' && (mnem[1] == 'r' || mnem[1] == 'p')) {
		...
		char *save = token;
		char *psave = p;	/* save mnemonic position */
		...
		if (cinfo[*token] & (S|T)) {
			...
			*p = 0;
			...
			p = psave;	/* restore to just "jr" or "jp" */
		}
		token = save;
	}
	*p = 0;
  ...
  struct oper *opnd;
  {
	...
	if (cinfo[*lptr] == IND) {
	  ...
	  if (opnd->type_o == t_reg) {
	    ...
	    /* @address - treat as direct address for indirect */
	    Prog_Error(E_OPERAND);
	  }
	  return(lptr);
	}

	/* expression or register */
	lptr = exp(lptr,opnd);
	skipb(lptr);

	/* check for indexed mode: value(reg) */
	if (cinfo[*lptr] == LP) {
	  struct oper regop;
	  opnd->disp_o = opnd->value_o;
	  lptr = exp(++lptr,&regop);
	  skipb(lptr);
	  if (cinfo[*lptr] == RP) lptr++;
	  else Prog_Error(E_OPERAND);
	  if (regop.type_o == t_reg) {
	    opnd->reg_o = regop.value_o;
	    opnd->type_o = t_x;
	  } else {
	    Prog_Error(E_REG);
	  }
	...
	/* plain register stays as t_reg */
	/* plain value/symbol is t_normal (direct address) */
	return(lptr);
}
	    ...
	    case SUB:	lptr = term(++lptr,&Arg2);
			...
			if (Arg2.sym_o)		/* if B is relocatable, */
			  if (Arg1->sym_o) {	/* and A is relocatable, */
			    if (Arg2.sym_o->csect_s != Arg1->sym_o->csect_s) break;
			    else {
			      Arg1->sym_o = NULL;
...
printop(o)
  ...
  {	fprintf(stderr,"operand %d: type=%s ",numops,tnames[o->type_o]);
	...
	fprintf(stderr,"value=%ld reg=%d disp=%ld\n",
